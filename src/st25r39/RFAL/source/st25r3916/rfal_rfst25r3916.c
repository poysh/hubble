...
LOG_MODULE_REGISTER(rfal_rfst25r3916);
/*
 ******************************************************************************
 * ENABLE SWITCHS
 ******************************************************************************
...
typedef union
{                                         /*  PRQA S 0750 # MISRA 19.2 - Both members are of the same type, just different names.  Thus no problem can occur. */
...
#define RFAL_FIFO_IN_LT_200 200U                                          /*!< Number of bytes in the FIFO when WL interrupt occurs while Tx                   */
#define RFAL_FIFO_OUT_LT_200 (ST25R3916_FIFO_DEPTH - RFAL_FIFO_IN_LT_200) /*!< Number of bytes sent/out of the FIFO when WL interrupt occurs while Tx          */
...
ReturnCode rfalSetBitRate(rfalBitRate txBR, rfalBitRate rxBR)
{
    ...
    case RFAL_MODE_POLL_PICOPASS:
        ...
        if (((gRFAL.rxBR != RFAL_BR_26p48) && (gRFAL.rxBR != RFAL_BR_52p97) && (gRFAL.rxBR != RFAL_BR_106) && (gRFAL.rxBR != RFAL_BR_212)) || ((gRFAL.txBR != RFAL_BR_1p66) && (gRFAL.txBR != RFAL_BR_26p48)))
        {
            return ERR_PARAM;
        }
...
ReturnCode rfalStartTransceive(const rfalTransceiveContext *ctx)
{
    ...
    if ((gRFAL.state >= RFAL_STATE_MODE_SET) /*&& (gRFAL.TxRx.state == RFAL_TXRX_STATE_INIT )*/)
    {
        ...
        { /* Exchange receive buffer with internal buffer */
            ...
            gRFAL.TxRx.ctx.flags = (uint32_t)RFAL_TXRX_FLAGS_CRC_TX_MANUAL | (uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP | (uint32_t)RFAL_TXRX_FLAGS_NFCIP1_OFF | (uint32_t)(gRFAL.nfcvData.origCtx.flags & (uint32_t)RFAL_TXRX_FLAGS_AGC_OFF) | (uint32_t)RFAL_TXRX_FLAGS_PAR_RX_KEEP | (uint32_t)RFAL_TXRX_FLAGS_PAR_TX_NONE;
...
extern void st25r_lowlayer_mutex_lock(void);
    st25r_lowlayer_mutex_lock();
    switch (gRFAL.state)
    {
...
extern void st25r_lowlayer_mutex_unlock(void);
    st25r_lowlayer_mutex_unlock();
...
static void rfalPrepareTransceive(void)
{
    ...
    maskInterrupts = (ST25R3916_IRQ_MASK_FWL | ST25R3916_IRQ_MASK_TXE |
                      ...
                      ST25R3916_IRQ_MASK_FWL | ST25R3916_IRQ_MASK_NRE |
...
static void rfalTransceiveTx(void)
{
    ...
    case RFAL_TXRX_STATE_TX_TRANSMIT: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through */
        ...
        gRFAL.fifo.expWL = RFAL_FIFO_OUT_LT_200;
...
static void rfalTransceiveRx(void)
{
    ...
    case RFAL_TXRX_STATE_RX_READ_DATA: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through */
        ...
        if (((gRFAL.TxRx.ctx.flags & (uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP) == 0U))
        {
            ...
            if ((gRFAL.fifo.bytesTotal + tmp) > 0U)
            {
        ...
        if (((RFAL_MODE_POLL_NFCV == gRFAL.mode) || (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) && (gRFAL.TxRx.status == ERR_BUSY))
        {
            ...
            if (((ERR_NONE == ret) || (ERR_CRC == ret)) && (((uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP & gRFAL.nfcvData.origCtx.flags) == 0U) && ((*gRFAL.nfcvData.origCtx.rxRcvdLen % RFAL_BITS_IN_BYTE) == 0U) && (*gRFAL.nfcvData.origCtx.rxRcvdLen >= rfalConvBytesToBits(RFAL_CRC_LEN)))
            {
...
ReturnCode rfalISO14443ATransceiveShortFrame(rfal14443AShortFrameCmd txCmd, uint8_t *rxBuf, uint8_t rxBufLen, uint16_t *rxRcvdLen, uint32_t fwt)
{
    ...
    { /* MISRA 15.6: mandatory brackets */
    };
    while (st25r3916IsGPTRunning())
    { /* MISRA 15.6: mandatory brackets */
    };
...
ReturnCode rfalListenStart(uint32_t lmMask, const rfalLmConfPA *confA, const rfalLmConfPB *confB, const rfalLmConfPF *confF, uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rxLen)
{
    ...
    if ((lmMask & RFAL_LM_MASK_NFCA) != 0U)
    {
        ...
        /*******************************************************************************/
        /* Set NFCID */
        ST_MEMCPY(pPTMem, confA->nfcid, RFAL_NFCID3_LEN);
        pPTMem = &pPTMem[RFAL_NFCID3_LEN]; /* MISRA 18.4 */
        ...
        case RFAL_LM_STATE_IDLE:
            ...
            if (!st25r3916CheckReg(ST25R3916_REG_OP_CONTROL, ST25R3916_REG_OP_CONTROL_en, ST25R3916_REG_OP_CONTROL_en))
            {
                ...
                if (!st25r3916CheckReg(ST25R3916_REG_AUX_DISPLAY, ST25R3916_REG_AUX_DISPLAY_osc_ok, ST25R3916_REG_AUX_DISPLAY_osc_ok))
                {
                    ...
                    st25r3916WaitForInterruptsTimed(ST25R3916_IRQ_MASK_OSC, 0);
                }
            }
            else
            {
...
ReturnCode rfalChipMeasureAmplitude(uint8_t *result)
{
    ...
    st25r3916WriteRegister(ST25R3916_REG_MODE, ST25R3916_REG_MODE_om_iso14443a | ST25R3916_REG_MODE_targ_init | ST25R3916_REG_MODE_tr_am_ook | ST25R3916_REG_MODE_nfc_ar_off);
    st25r3916WriteRegister(ST25R3916_REG_RX_CONF1, (reg_conf1 & ~ST25R3916_REG_RX_CONF1_ch_sel_AM));
    st25r3916WriteRegister(ST25R3916_REG_RX_CONF2, ((reg_conf2 & ~(ST25R3916_REG_RX_CONF2_demod_mode | ST25R3916_REG_RX_CONF2_amd_sel)) | ST25R3916_REG_RX_CONF2_amd_sel_peak));
