...
#if defined(ST25R391X_COM_SINGLETXRX) && !defined(RFAL_USE_I2C)
...
static void st25r3916comTx( const uint8_t* txBuf, uint16_t txLen, bool last, bool txOnly )
{
	...
	if( txLen > 0U )
	{
          platformSpiTxRx( (uint8_t*)txBuf, NULL, txLen );
	}
}
...
static void st25r3916comTxByte( uint8_t txByte, bool last, bool txOnly )
{
	st25r3916comTx( (const uint8_t* )&txByte, ST25R3916_REG_LEN, last, txOnly );
}
...
ReturnCode st25r3916ClrRegisterBits( uint8_t reg, uint8_t clr_mask )
{
	...
	if( ST25R3916_OPTIMIZE && (rdVal == (rdVal & ~clr_mask)) )
	{
		return ERR_NONE;
	}

	/* Write new reg value */
	return st25r3916WriteRegister(reg, (rdVal & ~clr_mask) );
}
...
{
	if(!(((signed char)reg >= ST25R3916_REG_IO_CONF1) && (reg <= (ST25R3916_SPACE_B | ST25R3916_REG_IC_IDENTITY))))
	{
		return false;
	}
	return true;
