...
{
  uint32_t t3NdefLen;
  uint8_t err = BSP_NFCTAG_ReadData((uint8_t* )&NDEF_Attr_Info,0,16);
  if(err != NFCTAG_OK)
  {
    return NDEF_ERROR;
  }
...
uint16_t NfcType3_ReadNDEF( uint8_t* pData )
{
  ...
  err = BSP_NFCTAG_ReadData(pData,16,length);
  if(err != NFCTAG_OK)
  {
    return NDEF_ERROR;
  }
...
{
  uint8_t err = BSP_NFCTAG_WriteData(pData,16,Length);
  if(err != NFCTAG_OK)
  {
    return NDEF_ERROR;
  }

  err = BSP_NFCTAG_ReadData((uint8_t* )&NDEF_Attr_Info,0,16);
  if(err != NFCTAG_OK)
  {
    return NDEF_ERROR;
  }
  ...
  err = BSP_NFCTAG_WriteData((uint8_t* )&NDEF_Attr_Info,0,16);
  if(err != NFCTAG_OK)
  {
    return NDEF_ERROR;
  }
