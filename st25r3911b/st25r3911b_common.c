...
static int osc_start(void)
{
	...
	LOG_INF("ST25R3911B_REG_OP_CTRL 0x%02x", reg);
...
static int st25r3911b_read_id()
{
	uint8_t id = 0x00;
	st25r3911b_reg_read(ST25R3911B_REG_IC_IDENTITY, &id);
	...
	LOG_INF("Rev 0x%02x", id & ST25R3916_REG_IC_IDENTITY_ic_rev_mask);
	return 0;
}
